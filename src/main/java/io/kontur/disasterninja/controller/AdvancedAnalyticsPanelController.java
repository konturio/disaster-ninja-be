...
public class AdvancedAnalyticsPanelController {
    ...
    @Operation(summary = "Calculate data for advanced analytics panel using insights-api service for provided indicators",
            ...
            description = "Calculate advanced analytics for polygon and requested layers. Executes GraphQL query PolygonStatistics/analytics/advancedAnalytics and returns a bunch of statistical functions for the list of provided indicators (numerators)")
    ...
    @Operation(summary = "Calculate data for advanced analytics panel using insights-api service",
            tags = {"Advanced Analytics Tab"},
            description = "Calculate advanced analytics for polygon. Executes GraphQL query PolygonStatistics/analytics/advancedAnalytics and returns a bunch of statistical functions for every available indicator (approx. 552 items)")
    @ApiResponse(responseCode = "200", description = "Successful operation",
            content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = AdvancedAnalyticsDto.class))))
    @PostMapping
